...
typedef struct _configStruct{
    unsigned char  configHeader [ CONFIG_HEADER_SIZE  ];
    unsigned char HIDDescriptor [ HID_DESCRIPTOR_SIZE ];
} ConfigStruct;
...
unsigned char ctrlTransferStage; // Holds the current stage in a control transfer

unsigned char HIDPostProcess;    // Set to 1 if HID needs to process after the data stage
unsigned char requestHandled;    // Set to 1 if request was understood and processed.

unsigned char *outPtr;           // Data to send to the host
code unsigned char *ROMoutPtr;    // Data to send to the host
unsigned char *inPtr;            // Data from the host
unsigned int wCount;            // Number of bytes of data
unsigned char transferType;	// 0=ram 1=rom

// HID Class variables
unsigned char hidIdleRate;
unsigned char hidProtocol; // [0] Boot Protocol [1] Report Protocol
unsigned char hidRxLen;    // # of bytes put into buffer

code unsigned char deviceDescriptor[] ={
    ...
    0x0B, 0x01, // idProduct (low byte), idProduct (high byte)
...
code unsigned char HIDReport[HID_REPORT_SIZE] = {
...
/*
code unsigned char stringDescriptor0[] ={
    0x04, STRING_DESCRIPTOR,
    0x09, 0x04,
};

code unsigned char stringDescriptor1[] ={
    24, STRING_DESCRIPTOR, // bLength, bDscType
    'V',0,'E',0,'N',0,'D',0,'O',0,'R',0,' ',0,'N',0,'A',0,'M',0,'E',0,
};

code unsigned char stringDescriptor2[] ={
    26, STRING_DESCRIPTOR,
    'P',0,'R',0,'O',0,'D',0,'U',0,'C',0,'T',0,' ',0,'N',0,'A',0,'M',0,'E',0,
};
code unsigned char stringDescriptor3[] ={
    28, STRING_DESCRIPTOR,
    'S',0,'E',0,'R',0,'I',0,'A',0,'L',0,' ',0,'N',0,'U',0,'M',0,'B',0,'E',0,'R',0,
};
*/
...
volatile unsigned char controlTransferBuffer[E0SZ];

// Put USB I/O buffers into dual port RAM.
#pragma udata usbram5 HIDRxBuffer HIDTxBuffer

// HID specific buffers
volatile unsigned char HIDRxBuffer[HID_OUTPUT_REPORT_BYTES];
volatile unsigned char HIDTxBuffer[HID_INPUT_REPORT_BYTES];
...
unsigned char HIDTxReport(unsigned char *buffer, unsigned char len){
#if !USE_MEMCPY
    unsigned char i;
#endif
...
unsigned char HIDRxReport(unsigned char *buffer, unsigned char len){
    hidRxLen = 0;
#ifdef DEBUG_PRINT
	...
	printf("HIDRxReport: %d: 0x%hx 0x%hx 0x%hx 0x%hx\r\n",
        (word)len, (unsigned char)buffer[0], (unsigned char)buffer[1], (unsigned char)buffer[2], (unsigned char)buffer[3]);
#endif
...
void HIDGetReport(void){
    unsigned char reportID = SetupPacket.wValue0;
...
void HIDSetReport(void){
    ...
    if (SetupPacket.wValue1 == 0x02){
        SetupOutputReport(reportID);
        requestHandled = 1; 
    }else if (SetupPacket.wValue1 == 0x03){
        SetupFeatureReport(reportID);
        requestHandled = 1;
    }
}
...
void ProcessHIDRequest(void){
    ...
    if (bRequest == GET_DESCRIPTOR){
        ...
        unsigned char descriptorType  = SetupPacket.wValue1;
        if (descriptorType == HID_DESCRIPTOR){
          ...
          printf("HID: HID_DESCRIPTOR\r\n");
            ...
            // HID descriptor.
            requestHandled = 1;
            outPtr = (code unsigned char *) &configDescriptor.HIDDescriptor;
        ...
        printf("HID: REPORT_DESCRIPTOR\r\n");
            ...
            // Report descriptor.
            requestHandled = 1;
            ROMoutPtr = (code unsigned char *)HIDReport;
...
static void GetDescriptor(void){
    ...
    if(SetupPacket.bmRequestType == 0x80){
        unsigned char descriptorType  = SetupPacket.wValue1;
        unsigned char descriptorIndex = SetupPacket.wValue0;
        if (descriptorType == DEVICE_DESCRIPTOR){
#ifdef DEBUG_PRINT
                printf("DEVICE_DESCRIPTOR\r\n");
#endif
                requestHandled = 1;
                ROMoutPtr = (code unsigned char *)&deviceDescriptor;
                wCount = DEVICE_DESCRIPTOR_SIZE;
		transferType=1;
        }else if (descriptorType == CONFIGURATION_DESCRIPTOR){
                requestHandled = 1;
                ROMoutPtr = (code unsigned char *)&configDescriptor;
                wCount = CFSZ;
		transferType=1;
#ifdef DEBUG_PRINT
                ...
                if (descriptorIndex == 0){
                    ROMoutPtr = (code unsigned char *) &stringDescriptor0;
                }else if (descriptorIndex == 1){
                    ROMoutPtr = (code unsigned char *)&stringDescriptor1;
                }else if (descriptorIndex == 2){
                    ROMoutPtr = (code unsigned char *)&stringDescriptor2;
                }else{
                    ROMoutPtr = (code unsigned char *)&stringDescriptor3;
                }
                wCount = *ROMoutPtr;
		transferType=1;
        }else{
...
static void GetStatus(void){
    ...
    unsigned char recipient = SetupPacket.bmRequestType & 0x1F;
#ifdef DEBUG_PRINT
    printf("GetStatus\r\n");
#endif
    ...
    if (recipient == 0x00){
        ...
        // Endpoint
        unsigned char endpointNum = SetupPacket.wIndex0 & 0x0F;
        unsigned char endpointDir = SetupPacket.wIndex0 & 0x80;
        requestHandled = 1;
        // Endpoint descriptors are 8 bytes long, with each in and out taking 4 bytes
        // within the endpoint. (See PIC datasheet.)
        inPtr = (unsigned char *)&ep0Bo + (endpointNum * 8);
        if (endpointDir){
            inPtr += 4;
        }
        if(*inPtr & BSTALL){
...
static void SetFeature(void){
    unsigned char recipient = SetupPacket.bmRequestType & 0x1F;
    unsigned char feature = SetupPacket.wValue0;
#ifdef DEBUG_PRINT
    printf("SetFeature\r\n");
#endif
    if (recipient == 0x00){
...
void ProcessStandardRequest(void){
    unsigned char request = SetupPacket.bRequest;
    ...
    if (request == SET_ADDRESS){
...
#endif
            requestHandled = 1;
            outPtr = (unsigned char*)&currentConfiguration;
            wCount = 1;
	    transferType=0;
        ...
        (request == SET_FEATURE)){
            ...
            requestHandled = 1;
            controlTransferBuffer[0] = 0;
            outPtr = (data unsigned char *)&controlTransferBuffer;
            wCount = 1;
            transferType=0;
...
void InDataStage(void){
    ...
    ep0Bi.Stat |= (unsigned char)((bufferSize & 0x0300) >> 8);
    ep0Bi.Cnt = (unsigned char)(bufferSize & 0xFF);
    ...
    inPtr = (unsigned char *)&controlTransferBuffer;
    if(transferType==1) {
...
void ProcessControlTransfer(void){   
    ...
    if (USTAT == 0){
        // Endpoint 0:out
        unsigned char PID = (ep0Bo.Stat & 0x3C) >> 2; // Pull PID from middle of BD0STAT
        if (PID == 0x0D){
